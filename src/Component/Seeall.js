    ...
    <div className="flex text-center justify-center items-center overflow-hidden">
      <div className="mt-2 p-1 mb-2 h-auto" style={{ width: "27rem" }}>
        ...
        <h1 className="font-bold text-white text-2xl">
          Karya's <span hidden={!Isupdated}>Updating...{ikk} sec.</span>{" "}
        </h1>
        <p>Numbers of Karyas - {karyas}</p>
        {todos.map((todo, index) => (
          <div
            key={index}
            className={`border-2 rounded-full border-solid border-red-500 w-auto h-auto mb-3 p-4 bg-yellow-500 hover:shadow-inner shadow-2xl z-10 shadow-zinc-700 ${
              Isupdated && doubleClickedIndex === index ? "blur-effect" : ""
            }`}
          >
            <input
            ...
            />
            <textarea
              className="mt-4 p-2 font-bold-2 font-bold text-orange-900 text-center"
              style={{ width: "17rem" }}
              disabled={isDisabled}
              onDoubleClick={() => handleDoubleClick(todo.Title, todo.Note, index)}
              onBlur={handleblurfield}
            ...
            <div className="w-auto h-auto float-right">
              ...
              <button
                ...
                className="w-auto p-2 h-10 border-2 border-dotted border-green-100 bg-green-500"
                hidden={!donebtn || doubleClickedIndex !== index}
              >
